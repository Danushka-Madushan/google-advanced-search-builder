...
const App: FC = () => {
  ...
  const setOp = useCallback((key: keyof SearchOperators, val: string | number) =>
  ...
  /* Mode switching — always resets search state to avoid conflicts */
  ...
  const handleSearch = () => { if (query.trim()) window.open(GOOGLE_BASE + encodeURIComponent(query), "_blank"); };

  const mkInput = (focused: boolean, themeColor: string = G.blue, themeLight: string = G.blueLight): React.CSSProperties => ({
    padding: "10px 14px", borderRadius: 8,
    border: `1px solid ${focused ? themeColor : G.border}`,
    outline: "none", fontSize: 14, fontFamily: "'Google Sans', Roboto, Arial, sans-serif",
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px 30px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, paddingBottom: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6 }}>
            <span style={{ fontSize: 28, fontWeight: 400, color: G.black, letterSpacing: "-0.5px", fontFamily: "'Google Sans', Roboto, Arial, sans-serif" }}>
              Advanced Search Builder
            </span>
            <span style={{
              fontSize: 11, fontWeight: 600, background: G.blueLight, color: "#1967D2",
              border: "1px solid #C5D9F9", padding: "3px 10px", borderRadius: 9999,
              fontFamily: "'Google Sans', Roboto, Arial, sans-serif", letterSpacing: "0.02em",
            }}>
              Query Builder
            </span>
          </div>
          <p style={{ fontSize: 14, color: G.grey, margin: 0, textAlign: "center", fontFamily: "'Google Sans', Roboto, Arial, sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, padding: "4px 0" }}>
          ...
          <button onClick={handleReset}
            style={{
              display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: G.grey,
              background: G.white, border: `1px solid ${G.border}`, padding: "8px 18px",
              borderRadius: 9999, cursor: "pointer", fontFamily: "'Google Sans', Roboto, Arial, sans-serif",
        ...
        {/* ─ Main Search Bar (Advanced mode only) ─ */}
        {mode === "advanced" && (
          <div style={{ position: "relative" }}>
            ...
            <input
              ...
              style={{
                ...
                borderRadius: 30, fontSize: 16, fontFamily: "'Google Sans', Roboto, Arial, sans-serif",
                color: G.black, background: G.white,
            ...
            <button onClick={handleSearch} disabled={!query.trim()}
              style={{
                ...
                fontSize: 14, fontWeight: 500, fontFamily: "'Google Sans', Roboto, Arial, sans-serif",
        ...
        {/* ─ URL Panel ─ */}
        <div style={{ borderRadius: 12, border: `1px solid ${G.border}`, background: G.white, overflow: "hidden", boxShadow: "0 1px 3px rgba(60,64,67,0.07)" }}>
          <button
            onClick={() => setUrlExpanded(v => !v)}
            onMouseEnter={() => setUrlPanelHov(true)}
            onMouseLeave={() => setUrlPanelHov(false)}
            style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "14px 20px", background: urlPanelHov ? "#F8F9FA" : "transparent", border: "none", cursor: "pointer", textAlign: "left", transition: "background 0.15s" }}>
            <Link2 size={15} style={{ color: G.blue }} />
            <span style={{ fontSize: 13, fontWeight: 500, color: G.textSecondary, flex: 1, fontFamily: "'Google Sans', Roboto, Arial, sans-serif", letterSpacing: "0.01em" }}>
              Generated Search URL
            </span>
            {query.trim() && (
              <span style={{ ...css.badge(G.blueLight, "#1967D2", "#C5D9F9"), marginRight: 4 }}>
                {query.trim().length} chars
              </span>
            )}
            {urlExpanded ? <Zap size={15} style={{ color: G.grey }} /> : <Search size={15} style={{ color: G.grey }} />}
          </button>

          {urlExpanded && (
            <div style={{ borderTop: `1px solid ${G.border}` }}>
              <div style={{ padding: "14px 20px", borderBottom: `1px solid #F8F9FA` }}>
                ...
                <div style={{ fontFamily: "'Roboto Mono', monospace", fontSize: 13, color: G.greenDark || "#137333", background: "#F6FBF8", borderRadius: 8, padding: "12px 16px", minHeight: 42, wordBreak: "break-all", border: "1px solid #D6EFE0" }}>
              ...
              <div style={{ padding: "14px 20px" }}>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  <div style={{ flex: 1, fontFamily: "'Roboto Mono', monospace", fontSize: 12, background: "#F8F9FA", borderRadius: 8, padding: "12px 16px", wordBreak: "break-all", border: `1px solid ${G.border}`, minHeight: 42 }}>
                    {query.trim()
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
                    <button onClick={handleCopy} disabled={!query.trim()}
                      style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 18px", borderRadius: 8, background: G.white, border: `1px solid ${G.border}`, cursor: query.trim() ? "pointer" : "not-allowed", fontSize: 13, color: G.textSecondary, fontFamily: "'Google Sans', Roboto, Arial, sans-serif", opacity: query.trim() ? 1 : 0.4, fontWeight: 500, transition: "all 0.15s" }}
                    ...
                    <button onClick={handleSearch} disabled={!query.trim()}
                      style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 18px", borderRadius: 8, background: query.trim() ? G.blue : "#F1F3F4", border: "none", cursor: query.trim() ? "pointer" : "not-allowed", fontSize: 13, color: query.trim() ? G.white : G.grey, fontFamily: "'Google Sans', Roboto, Arial, sans-serif", fontWeight: 500, transition: "background 0.15s" }}
                      onMouseEnter={e => { if (query.trim()) e.currentTarget.style.background = "#1A73E8"; }}
                      onMouseLeave={e => { if (query.trim()) e.currentTarget.style.background = G.blue; }}>
        ...
        {mode === "super" && (
          <div style={{
            ...
            background: G.white, overflow: "hidden",
            boxShadow: "0 2px 12px rgba(123,97,255,0.12)",
          }}>
            {/* Header */}
            <div style={{ padding: "18px 24px", background: "linear-gradient(135deg, #F0EDFF 0%, #F8F7FF 100%)", borderBottom: "1px solid #DDD8FF", display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ width: 10, height: 10, borderRadius: "50%", background: "#7B61FF", flexShrink: 0, boxShadow: "0 0 0 3px #E8E4FF" }} />
              <Folder size={18} style={{ color: "#7B61FF" }} />
              <span style={{ fontWeight: 600, fontSize: 15, color: "#3D0099", fontFamily: "'Google Sans', Roboto, Arial, sans-serif", flex: 1 }}>
                Super File Search
              </span>
              <span style={{ ...css.badge("#EDE9FF", "#5B21B6", "#C8BFFF"), fontSize: 12 }}>
                Open Directories
              </span>
            </div>
            ...
            <div style={{ padding: "20px 24px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
              {/* Info banner */}
              <div style={{ fontSize: 13, color: "#3D0099", background: "#F5F2FF", borderRadius: 8, padding: "12px 16px", border: "1px solid #DDD8FF", fontFamily: "'Google Sans', Roboto, Arial, sans-serif", lineHeight: 1.7 }}>
                Finds direct download links in open directory listings using{" "}
                <code style={{ fontFamily: "'Roboto Mono', monospace", background: "#EDE9FF", color: "#5B21B6", padding: "2px 6px", borderRadius: 4, fontSize: 12 }}>
                  intitle:index.of
                </code>{" "}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {FILE_TYPES.map(({ Icon, label, value }) => {
                    ...
                    return (
                      <button key={value || "__any__"} onClick={() => setSfsFileType(value)}
                        style={{
                          padding: "9px 18px", borderRadius: 9999, fontSize: 13,
                      ...
                      >
                        <Icon size={15} color={sel ? "#7B61FF" : G.grey} />
                        {label}
                      </button>
                    );
                  })}
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
                  ...
                  <input
                    ...
                    placeholder="e.g.  The.Blacklist.S01.E01  ·  1984 George Orwell  ·  GTA V"
                    style={{
                      width: "100%", padding: "12px 16px", borderRadius: 8, fontSize: 14,
                      border: `1px solid ${sfsFocused ? "#7B61FF" : G.border}`,
                      outline: "none", fontFamily: "'Google Sans', Roboto, Arial, sans-serif",
                      color: G.black, background: G.white,
                ...
                <button onClick={handleSearch} disabled={!sfsQuery.trim()}
                  style={{
            ...
            <Section title="Basic Operators" icon={Hash} theme="blue">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <select value={ops.priceCurrency} onChange={e => setOp("priceCurrency", e.target.value)}
                    style={{ padding: "10px 14px", borderRadius: 8, border: `1px solid ${G.border}`, fontFamily: "'Google Sans', Roboto, Arial, sans-serif", fontSize: 14, color: G.black, background: G.white, cursor: "pointer", flexShrink: 0 }}>
              ...
              <div style={{ gridColumn: "1 / -1", display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  <input type="text" value={ops.aroundTerm1} onChange={e => setOp("aroundTerm1", e.target.value)}
                    ...
                    style={{ flex: 1, minWidth: 110, ...mkInput(ar1F, "#F29900", G.yellowLight), borderColor: ar1F ? "#F29900" : G.border }} />
                  <div style={{ display: "flex", alignItems: "center", gap: 4, background: G.yellowLight, border: "1px solid #FDE293", padding: "9px 14px", borderRadius: 8, flexShrink: 0 }}>
                  ...
                  <input type="text" value={ops.aroundTerm2} onChange={e => setOp("aroundTerm2", e.target.value)}
                    ...
                    style={{ flex: 1, minWidth: 110, ...mkInput(ar2F, "#F29900", G.yellowLight), borderColor: ar2F ? "#F29900" : G.border }} />
                </div>
        ...
        <div style={{ borderRadius: 12, border: `1px solid ${G.border}`, background: G.white, overflow: "hidden", boxShadow: "0 1px 3px rgba(60,64,67,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "16px 20px", borderBottom: `1px solid #F8F9FA` }}>
            <Layers size={16} style={{ color: G.blue }} />
            <span style={{ fontSize: 13, fontWeight: 500, color: G.textSecondary, fontFamily: "'Google Sans', Roboto, Arial, sans-serif", flex: 1 }}>
          ...
          <div style={{ padding: "8px 16px 20px", display: "flex", flexDirection: "column" }}>
            {[
              ...
              <div key={r.label}
                style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "10px 4px", borderRadius: 8, transition: "background 0.1s", cursor: "default" }}
                onMouseEnter={e => e.currentTarget.style.background = "#F8F9FA"}
                onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                <ChevronRight size={14} style={{ color: G.grey, marginTop: 2, flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 13, color: G.textSecondary, margin: "0 0 3px", fontFamily: "'Google Sans', Roboto, Arial, sans-serif", fontWeight: 500 }}>
                    {r.label}
                  </p>
                  <button
                    onClick={() => {
                    ...
                    style={{ fontFamily: "'Roboto Mono', monospace", fontSize: 12, color: G.blue, background: "none", border: "none", cursor: "pointer", padding: 0, wordBreak: "break-all", textAlign: "left", transition: "color 0.15s" }}
